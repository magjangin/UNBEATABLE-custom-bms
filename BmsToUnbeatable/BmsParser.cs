...
{
    private static readonly Regex ExtBpmRegex = new(@"^#BPM([0-9A-Za-z]{2})\s+(.+)$", RegexOptions.Compiled);
    private static readonly Regex InitialBpmRegex = new(@"^#BPM\s+(.+)$", RegexOptions.Compiled);
    private static readonly Regex StopRegex = new(@"^#STOP([0-9A-Za-z]{2})\s+(.+)$", RegexOptions.Compiled);
